...
size_t mam_psk_t_set_size(mam_psk_t_set_t const set) { return HASH_COUNT(set); }
...
bool mam_psk_t_set_find(mam_psk_t_set_t const set, mam_psk_t const *const value, mam_psk_t_set_entry_t **entry) {
  if (set == NULL) {
    return false;
  }
